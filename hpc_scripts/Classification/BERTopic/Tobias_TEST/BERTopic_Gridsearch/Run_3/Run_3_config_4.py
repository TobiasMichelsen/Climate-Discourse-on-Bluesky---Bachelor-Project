...
# Load clustered data from previous run2

base_dir = "/home/absz/BachProj/Classification"
cluster_input_path = os.path.join(base_dir, "run3/logs/cleaned_run3_input.json")
df_clustered = pd.read_json(cluster_input_path, lines=True)
df_clustered = df_clustered.loc[df_clustered.topic == -1]

print(f"Loaded {len(df_clustered)} documents from cleaned clusters from run2", flush=True)


# --- Hyperparameters ---

#Combinations: 32, 2 distance metrics, low gran

embedding_models = ["all-MiniLM-L6-v2"]
min_cluster_sizes = [75,150]
min_samples_vals = [15,30]
distance_metrics = ["cosine","euclidean"]
umap_neighbors = [5,15]
umap_components = [7,14]
umap_min_dist = [0.0]
nr_topics = 15
...
# --- Logging Setup ---
log_path = os.path.expanduser("run3/logs/bertopic_grid_log_3_4.csv")
...
# --- Embedding ---
texts_to_embed = df_clustered["text"].tolist()
seqs = df_clustered["seq"].tolist()
...
for embed_model in embedding_models:
    ...
    for (min_cluster_size, min_samples, metric,
        ...
        try:
            ...
            df_result.to_json("run3/logs/clusters_run3_4.json", orient="records", lines=True)
