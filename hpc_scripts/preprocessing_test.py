...
MIN_TEXT_LEN = 60
posts_folder = "../data/backups_test/posts"
output_folder = "../data/filtered/filtered_test"
...
MAX_RUNTIME = datetime.timedelta(minutes = 3)
...
def clean(post_file):
    ...
    # print(f"df shape after min text len cutoff: {df.shape}")

    df["text"] = df["text"].apply(remove_non_latin)  
    df = df.dropna(subset=["text"])
    df = df[df["text"].apply(len) >= MIN_TEXT_LEN]
    # print(f"df shape after non latin removal: {df.shape}")
...
def main():
    ...
    if os.path.exists(PARTIAL_DF_PATH):
        print("Resuming from previous partial concat_df...")
    ...
    while True:
        ...
        if elapsed_time > MAX_RUNTIME:
            print("Max job time reached. Saving partial df and exiting...")
            concat_df.to_json(PARTIAL_DF_PATH, orient="records", lines=False, force_ascii=False, indent=2)
            print(f"Saved {concat_df.shape[0]} rows to {PARTIAL_DF_PATH}")
            ...
            for filename in os.listdir(posts_folder):
                ...
                if filename.startswith("posts_backup_") and filename.endswith(".json"):
                    ...
                    if concat_df.shape[0] >= 10000:
                        ...
                        df_to_save = concat_df.head(100000)
                        df_to_save.to_json(f"{output_folder}/{output_filename}", orient="records", lines=False, force_ascii=False, indent=2)
                        print(f"{output_filename} saved containing 100k rows, YAY")
                        concat_df = concat_df[100000:]
        if not new_file_found:
            print("No new files found. Sleeping for 5 mins..\n")
            time.sleep(10)
    print("Job finished processing")
#RUN            
if __name__ == "__main__":
    main()

#dependencies ( are they all python included?)
#change MAX runtime
